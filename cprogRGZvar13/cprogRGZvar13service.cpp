...
int Server()
{
	...
	while (1)
	{
		...
		if (((bytes_recv = recv(my_sock, buff, sizeof(buff) - 1, 0)) > 0) //buff - строка от клиента
			...
			{
				char fileName[128] = "C://Users/Nikita/source/repos/cprogRGZvar13/cprogRGZvar13service/x64/Debug/";
				...
				ZeroMemory(SeqFromCMD, sizeof(SeqFromCMD));
				...
				if (space == 0) {
					...
					while (fileName[index] != '\0') {
						SeqFromCMD[newIndex] = fileName[index];
						index++;
						newIndex++;
					}
				}
				...
				if ((strlen(SeqFromCMD) != 2) || (SeqFromCMD[0] >= '0' && SeqFromCMD[0] <= '9') || (SeqFromCMD[1] >= '0' && SeqFromCMD[1] <= '9'))
					isSequenceCorrect = false;

				if (emptySeq == true || isSequenceCorrect == false) {
					sprintf(message2, "(Server)Wrong or missed parameter");
				...
				if (key == false) {
					...
					while (ReadFile(hIn, Buffer, BUF_SIZE, &nIn, NULL) && nIn > 0) {
						...
						{
							if ((Buffer[i] == SeqFromCMD[0]) && (Buffer[i + 1] == SeqFromCMD[1])) {
									Buffer[i] = '#';
									Buffer[i + 1] = '@';
									total++;
							}
						...
						sprintf(message, "(Server): file:%s, sequence:%s, changes:%d\n", nameoffile, SeqFromCMD, total);
						addLogMessage(message);
